...
## Repository
GitHub: https://github.com/Chandan24-cell/FinTrace-Financial-Fraud-Intelligence-Investigation

## Approach
**Two-tier intelligence.** 12 deterministic rule modules (M0–M11) feed a 4-detector ML ensemble. The rules catch what they were designed for, the ML learns the optimal combination, the anomaly detectors catch what neither knows about. (The PRD specified 6 detectors; D1/D2 were dropped as redundant — see `docs/SYSTEM_DESIGN.md` §5.5 for the sufficiency analysis.)
...
### Additional
- AI / ML
- Cyber Security (financial fraud forensics)
- Cloud

## Sponsored Track
**Neo4j Track** — Neo4j is the single data store. Every fraud signal, every audit log, every evidence chain lives in the graph. GDS SCC powers ITC carousel detection. GDS WCC powers entity resolution. Belief propagation Cypher writes `CONNECTED_TO_CRITICAL` edges.
...
## Deliverables
- **GitHub Repository:** https://github.com/Chandan24-cell/FinTrace-Financial-Fraud-Intelligence-Investigation
...
- **Demo Video:** to be added after Week 4 rehearsals (PRD §10 Day 28)
- **System Design:** see `docs/SYSTEM_DESIGN.md` — full technical reference with the rationale behind every layer (data sources, graph schema, all 12 rule modules, the 4-detector ML ensemble + sufficiency analysis, calibration, coverage matrix, personas, honest gaps).
- **PRD:** see `FinTrace_Final.docx`

## Data lineage — every signal traces back to a public-record source
Every fraud signal FinTrace fires is backed by a publicly-published government data source. The live `/sources` page on the deployed app (https://fintrace.vercel.app/sources — no login required) shows the full inventory with last-refreshed timestamps and record counts read from disk at request time. Below is the same table, in summary.
...
| NCLT CP(IB) admitted proceedings | Court record | Real case numbers (C.P.(IB) 4258/MB/2019 etc.) | M9 override floor ≥ 75 | Auto via nclt-admitted scraper (planned) |
...
## Final Words
If time runs out in Week 4, cut the Upload Portal and Benchmark screen. Never cut the IL&FS demo, the Graph Explorer with evidence provenance, the ITC carousel view, or the DHFL evergreening view. Those four things are FinTrace. Everything else is polish.
